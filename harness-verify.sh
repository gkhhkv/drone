...
echo -e "${YELLOW}[1/6] Claude Code Steering 文件${NC}"
check_file ".claude/CLAUDE.md"
echo ""

# ── 2. Hooks───────────────────────────────────────────
echo -e "${YELLOW}[2/6] Claude Code Hooks 自动化体系${NC}"
check_file ".claude/settings.local.json"
check_file ".claude/hooks/harness-reminder.ps1"
echo ""

# ── 3. Maven 构建配置──────────────────────────────────
echo -e "${YELLOW}[3/6] Maven 构建配置${NC}"
check_file "pom.xml"
...
# ── 4. CI/CD 配置─────────────────────────────────────
echo -e "${YELLOW}[4/6] CI/CD 配置${NC}"
check_file ".github/workflows/ci-verify.yml"
echo ""

# ── 5. harness-collab 文档体系────────────────────────
echo -e "${YELLOW}[5/6] harness-collab 文档体系${NC}"
...
if [ $ERRORS -eq 0 ]; then
  ...
  echo "新建 Spring Boot 子项目后，在 Claude Code 中打开工作区根目录即可开始受监管的开发。"
else
