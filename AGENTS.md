...
本文件是 Spring Boot Harness 工作区的 AI 协作协议入口。完整协议定义在 [harness-collab/AGENTS.md](harness-collab/AGENTS.md)，所有在本工作区内工作的 Claude Code 必须遵守该协议。

---

## Harness 监管机制

Harness 通过两种机制对工作区内的所有子项目实施自动监管：

**CLAUDE.md 自动加载**

`.claude/CLAUDE.md` 由 Claude Code 在每次会话启动时自动加载。无论开发者在工作区的哪个子目录下工作，以下约束始终生效：

- 四层架构约束、编码标准
- 测试分层策略、覆盖率要求
- API 文档同步规则
- AI 协作行为规范、交付标准
- Dev_Lifecycle 六阶段流程及门禁

**Hooks 自动触发**

`.claude/settings.local.json` 和 `.claude/hooks/` 下的 Hook 在文件编辑事件发生时自动触发：
...
## Claude Code 核心行为约束（快速参考）

在本工作区内，Claude Code 必须遵守以下五条核心规则：
